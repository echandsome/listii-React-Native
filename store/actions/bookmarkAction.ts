...
import {  addItem, updateItem, removeItem, 
    setAllItemsFalse, setAllItemsTrue, removeItemsFalse, removeItemsTrue } from '@/store/reducers/bookmarkReducer';
...
export async function updateItemByDB(nData: any, dispatch: Dispatch) {
    ...
    if (userId) {
        ...
        if (toggle) {
            ...
            const {data, error} = await supabase.from(tbl_names.items).update({ name: newItem.name, link: newItem.path, checked: newItem.is_check?? false }).eq('id', newItem.id);
